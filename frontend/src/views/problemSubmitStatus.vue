...
export default {
  ...
  methods: {
    ...
    rowClick(rowIndex, rowData, column) {
      switch (column.field) {
        ...
        case "submitTime":
          ...
          } else if (this.tableConfig.tableData[rowIndex].uid != localStorage.USER_ID) {
            let api = this.Url.userDetail;
            this.$axios.get(api+localStorage.USER_ID)
                .then(res => {
                  let resData = res.data;
                  console.log(resData.data.isAdmin);
                  if(resData.data.isAdmin === 2){ //如果是教师用户，可以查看所有人代码
    ...
    getSolutionList() {
      this.isLoading = true;
      let apiUrl = this.Url.problemSubmitStatus;
      this.$axios
