        ...
        <el-row>
          <el-col :span="12">
            ...
            <select class="newStudentList" multiple="multiple" size="8">
              <option v-for="item in newStudentNoList">{{ item.studentNo +"-" + item.username }}</option>
            </select>
          </el-col>
          <el-col :span="12">
            <p class="center">
...
import {getUserList, deleteUser, createUser, importUser} from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    async onNewUserBatchConfirmed() {
      this.newUserBatchConfirmDialogVisible = false
      this.newUserBatchProgressDialogVisible = true
      for(this.flag = 0; this.flag < this.newStudentNoList.length; this.flag++) {
        console.log("列表"+this.newStudentNoList[this.flag].username)
        await this.addUserInBulk(this.newStudentNoList[this.flag])
        //i--;
      }

      // for (const studentNo of this.newStudentNoList) {
      //   console.log("列表："+studentNo.studentNo)
      //   await this.addUserInBulk(studentNo)
      // }
    },
    onCancelSearch() {
      this.inSearch = false
    ...
    async addUserInBulk(student) {
      // const apiUrl = this.baseApiUrlConfig.userAddExcel
      // let postData = {
      //   username: student.username,
      //   studentNo: student.studentNo,
      //   email: student.studentNo + '@ncu.edu.cn',
      //   password: student.studentNo
      // }
      await this.handleResponse(importUser(student.username, student.studentNo, student.studentNo + "@ncu.edu.cn", student.studentNo, student.className), '导入用户',
        (res) => {
          console.log(res.message)
          if (res.message !== 'false'){
            //const studentNoIndexOf = this.newStudentNoList.indexOf(student)

            for(let i = 0; i < this.newStudentNoList.length; i++){
              if(this.newStudentNoList[i].studentNo === res.message){
                this.newStudentNoList.splice(i, 1)
                this.studentNoAddedList.push(student.studentNo + " " + student.username)
                this.flag--; //暂时没有想明白，删除之后，居然后续添加的就会跳过删除元素的后一个（只能添加1，3，5，7，9···），所以这里让下标往回走一个，可以解决这个问题
                break;
              }
            }
